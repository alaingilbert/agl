...
func main() {
    a := []int{1, 2, 3}
	assertEq(a.Iter().Position(|x| x == 2)?, 1)
	assertEq(a.Iter().RPosition(|x| x == 3)?, 2)
}`
	tassert.NotPanics(t, func() { testGenOutput(src) })
}
...
func main() {
	...
	it := a.Iter()
	assertEq(it.Len(), 5)
	assertEq(it.RFind({ $0 == 4 })?, 4)
	assertEq(it.RFind({ $0 == 3 })?, 3)
	assert(it.RFind({ $0 == 3 }).IsNone())
}`
	tassert.NotPanics(t, func() { testGenOutput(src) })
}
...
func main() {
	a := []int{1, 2, 3, 4}
	it := a.Iter()
	assertEq(it.AdvanceBy(2)!, void)
	assertEq(it.Next()?, 3)
	assertEq(it.AdvanceBy(0)!, void)
	assert(it.AdvanceBy(100).IsErr())
}`
	tassert.NotPanics(t, func() { testGenOutput(src) })
}
...
func main() {
	a := []int{3, 4, 5, 6}
	it := a.Iter()
	assertEq(it.AdvanceBackBy(2)!, void)
	assertEq(it.NextBack()?, 4)
	assertEq(it.AdvanceBy(0)!, void)
	assert(it.AdvanceBy(100).IsErr())
}`
	tassert.NotPanics(t, func() { testGenOutput(src) })
