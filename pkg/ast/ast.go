...
func (r RangeExpr) Pos() token.Pos { return r.Start.Pos() }
func (r RangeExpr) End() token.Pos { return r.End_.End() }
...
func (x *Ident) End() token.Pos {
	if x.Mutable.IsValid() {
		...
		return token.Pos(int(x.Mutable) + int(x.NamePos-x.Mutable) + len(x.Name))
	}
...
func NewIdent(name string) *Ident { return &Ident{token.NoPos, name, token.NoPos, nil} }
...
type (
	...
	ForStmt struct {
		...
		WhereCond Expr      // optional condition for "for x in y if cond"; or nil
		Body      *BlockStmt
	}
