...
func assert(cond bool, msg string) {
	if !cond {
		panic("agl/parser internal error: " + msg)
	}
}
...
func (p *parser) parseIdentOrOp() *ast.Ident {
	...
	case token.EQL, token.LSS, token.GTR, token.LEQ, token.GEQ,
		token.ADD, token.SUB, token.MUL, token.QUO, token.POW, token.REM, token.IN, token.RANGEOP, token.RANGEOPEQ,
...
func (p *parser) parseBody2(hasPipe bool) ast.Stmt {
	if p.trace {
		defer un(trace(p, "Body2"))
	}
	if hasPipe && p.tok != token.LBRACE {
		return p.parseStmt()
	}
	lbrace := p.expect(token.LBRACE)
...
func (p *parser) parseShortFuncLit() ast.Expr {
	...
	p.exprLev++
	body := p.parseBody2(pipe1Pos.IsValid())
	p.exprLev--
