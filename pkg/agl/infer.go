...
func (infer *FileInferrer) inferGoExtensions(expr *ast.CallExpr, idT, oidT types.Type, exprT *ast.SelectorExpr) {
	...
	case types.IntType:
		...
		case "String":
			...
			fnT = infer.env.GetFn("agl1.Int.String")
		}
	...
	case types.SetType:
		...
		case "Map":
			...
			clbFnT := fnT.GetParam(1).(types.FuncType)
			if len(expr.Args) < 1 {
				return
			}
			exprArg0 := expr.Args[0]
	...
	case types.ArrayType:
		...
		if fnName == "Filter" {
			...
			mapFnT := infer.env.GetFn("agl1.Vec.FilterMap").T("T", idTT.Elt)
			clbFnT := mapFnT.GetParam(1).(types.FuncType)
			if len(expr.Args) < 1 {
				return
			}
			...
			"Insert", "Pop", "PopFront", "Len", "IsEmpty", "__ADD", "RemoveFirst"}) {
