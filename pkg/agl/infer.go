			...
			if infer.mutEnforced && len(expr.Args) > 0 {
				for i, pp := range v.Params {
					...
					if TryCast[types.MutType](pp) {
						...
						case *ast.BasicLit, *ast.CallExpr, *ast.CompositeLit, *ast.BinaryExpr:
							// Literals, call expressions, composite literals, and binary expressions don't need mutability checks
...
func (infer *FileInferrer) inferGoExtensions(expr *ast.CallExpr, idT, oidT types.Type, exprT *ast.SelectorExpr) {
	...
	case types.IntType, types.UntypedNumType, types.I64Type, types.I32Type, types.I16Type, types.I8Type,
		types.U8Type, types.U16Type, types.U32Type, types.U64Type, types.UintType, types.F32Type, types.F64Type:
		envFnName := "agl." + numericTypeName(idTT) + "." + fnName
		if v := infer.env.GetNameInfo(envFnName); v != nil {
			info = v
			fnT = infer.env.GetFn(envFnName).IntoRecv(idTT)
		}
		...
		case "First":
			if len(expr.Args) > 0 {
				...
				case *ast.FuncLit, *ast.ShortFuncLit:
					...
					if !infer.checkLambdaArg(exprArg0, ft, exprPos) {
						return
					}
					infer.SetType(exprT.Sel, fnT, WithDesc(info.Message))
				}
	...
	case types.ArrayType:
		...
		if fnName == "Filter" {
			...
			"Insert", "Pop", "PopFront", "__ADD", "RemoveFirst", "Description"}) {
			...
			fnT.Params = fnT.Params[1:]
			infer.SetType(exprT.Sel, fnT)
		} else if InArray(fnName, []string{"Joined", "String"}) {
...
// checkLambdaArg type-checks a callback argument against the inferred function type want.
...
// checkLambdaArgWithReturn is checkLambdaArg for callbacks whose return type specializes
// the method's generic result (e.g. Map's R). A short function literal is inferred and
// onReturn is called with its concrete return type; an expression that already has a
...
func (infer *FileInferrer) rangeExpr(expr *ast.RangeExpr) {
	...
	if expr.Op == token.RANGEOPEQ {
		infer.SetType(expr, types.RangeInclusiveType{Typ: t})
	} else {
		...
		if stmt.Init == nil && stmt.Cond != nil && stmt.Post == nil &&
			...
			case types.StructType:
				if v.Name == "Sequence" {
					...
					infer.SetType(cond.X, t)
				} else if v.Name == "Rev" && len(v.TypeParams) > 0 {
					// Rev[T] is an iterator over T
					t = v.TypeParams[0]
					infer.SetType(cond.X, t)
				} else {
